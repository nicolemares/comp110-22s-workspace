...
def select_rows(data_dict: dict[str, list[str]], title: str, int_title: str, value: str) -> list[int]:
    com_sci_list: list[int] = []
    i: int = 0
    while i < len(data_dict[title]):
        empty_dict: dict[str, int] = {}
        if data_dict[title][i] == value:
            interest_numbers = int(data_dict[int_title][i])
            com_sci_list.append(interest_numbers)
        i = i + 1
    return com_sci_list


def select_other_rows(data_dict: dict[str, list[str]], title: str, int_title: str, value: str) -> list[int]:
    i: int = 0
    empty_list: list[int] = []
    while i < len(data_dict[title]):
        if data_dict[title][i] != value:
            empty_list.append(int(data_dict[int_title][i]))
        i = i + 1
    return empty_list


def average(data_list: list[int]) -> float:
    i: int = 0
    result: int = 0
    while i < len(data_list):
        result = result + data_list[i]
        i += 1
    result = result // len(data_list)
    return result